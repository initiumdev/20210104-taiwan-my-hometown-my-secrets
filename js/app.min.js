	...
	var init = function(){
		new Vue({
			...
			computed: {
				puppetsGroup(){
					switch(this.question){
						case '1':
							return _.chain(this.puppets)
									.groupBy('independent_pos')
									.value()
							break;
						case '2':
							return _.chain(this.puppets)
			...
			methods: {
				// async fetchData(){
				// 	try {
				// 		const res = await fetch('https://spreadsheets.google.com/feeds/list/1bvYB1LQYhDZaxJ68adB956Y-uowWWokgVr_9yyj_odM/1/public/values?alt=json', {method: 'GET',})
				// 		const data = await res.json()
				// 		this.puppets = data.feed.entry
				// 	} catch(e) {
				// 		console.error(e)
				// 	} finally {
				...
				kickPuppet(uuid){
					if(this.question !== '0'){
						this.player.id = String(uuid)
						this.infoDisplay(true)
						this.playerData()
						this.playerDisplay(true)
						this.playerAudio()
						if(this.player.egg){
							this.eggDisplay(true)
						}
					}
				},
				infoDisplay(boolean){
					if(boolean){
						...
						const target = '#puppet-'+ this.player.id
						tl.set('.puppet', {className: "puppet unactive"})
						  .set(target, {className: "puppet active"})
					} else {
						gsap.set('.puppet', {className: "puppet"})
					}
				},
				playerData(){
					const data = this.puppets.find(i => i.uuid === this.player.id)
					switch(this.question){
						case '1':
							this.player.lyrics = data.independent_lyrics
							this.player.time = data.independent_time
							this.player.egg = data.independent_egg
							break;
						case '2':
							this.player.lyrics = data.wars_lyrics
							this.player.time = data.wars_time
							this.player.egg = data.wars_egg
							break;
						case '3':
							this.player.lyrics = data.flag_lyrics
							this.player.time = data.flag_time
							this.player.egg = data.flag_egg
							break;
					}
				},
				...
				eggDisplay(boolean){
					if(boolean){
						gsap.set('.egg',{'display': 'block'})
						this.$refs.eggContent.scrollTop = 0
					} else {
						gsap.set('.egg',{'display': 'none'})
					}
				},
				playerAction(val){
					switch(val){
				...
				introAction(val){
					switch(val){
						case 'play':
							...
							break;
						case 'stop':
							this.intro.current.stop()
							break;
					}
				},
				playerAudio(){
					const sound = new Howl({
						src: ['audio/'+this.player.id+'-'+this.question+'.mp3'],
						preload: 'metadata',
						onplay: ()=>{
							this.player.state = 'play'
							this.timeline.lyrics.play()
						},
						onpause: ()=>{
							this.player.state = 'pause'
							this.timeline.lyrics.pause()
						},
						onstop: ()=>{
							this.player.state = 'stop'
							this.infoDisplay(false)
							this.playerDisplay(false)
							this.eggDisplay(false)
							this.timeline.lyrics.kill()
					...
					const time = Number(this.player.time)+1
					tl.add(()=>{
						if(this.player.current){
					  ...
					  .add(()=>{
						this.player.current = sound
						this.timeline.lyrics = gsap.fromTo('.player .lyrics .text', {x: '0%'}, {x: '-100%', duration: time, ease: "none", paused: true})
				...
				introAudio(){
					const sound = new Howl({
						src: ['audio/intro.mp3'],
						onplay: ()=>{
							this.intro.state = 'play'
						},
						onstop: ()=>{
							this.intro.state = 'stop'
						},
						onend: ()=>{
							this.introAction('stop')
						}
					});
					this.intro.current = sound
				},
				stIndex(){
					...
					const tl1 = gsap.timeline({
						...
						onComplete: () => {
							gsap.to('.loading .tips', {opacity: 0, duration: .5})
							this.introAction('stop')
						}
					})
					tl1.to('.loading .content .text', {opacity: 0, y: -30, duration: 1})
					   .set('.loading .content .audio', {'display': 'flex'})
					   .from('.loading .content .audio', {opacity: 0, y: 30, duration: 1})
					...
					tl2.from('.z-6', {opacity: 0, duration: .1})
					  .from('.intro-1 p', {stagger: 1, opacity: 0, y: 30, duration: 2})
					  .to('.intro-1', {opacity: 0, duration: .5})
					  .from('.intro-2 h3', {opacity: 0, y: 30, duration: 1})
					  .from('.intro-2 figure', {stagger: 0.33, opacity: 0, y: 30, duration: 1})
					  
					  .set('.z-5', {display: 'block'}, '+=3')
					  .set('.z-6', {display: 'none'})
					  .add(() => {
						this.question = '0'
					  })
					  .add(() => {
					  ...
					  .set('.z-4', {display: 'block'}, '+=3')
					  .set('.z-5', {display: 'none'})
					  .add(() => {
						this.question = '1'
					  })
					  .add(() => {
					  ...
					  .set('.z-3', {display: 'block'}, '+=3')
					  .set('.z-4', {display: 'none'})
					  .add(() => {
						this.question = '2'
					  })
					  .add(() => {
					  ...
					  .set('.z-2', {display: 'block'}, '+=3')
					  .set('.z-3', {display: 'none'})
					  .add(() => {
						if(this.player.current){
						  this.playerAction('stop')
						}
					...
					tl3.set('.puppets', {className: "puppets end"}, 0)
					  .from('.credit .info', {y: 30, rotate: 10, opacity: 0, duration: .2}, .8)
				},
				handleResize() {
				...
				hozMove(e){
					if(this.window.width < 1440 && this.window.width > 768){
						this.clientX = e.clientX
						let percent = this.clientX / this.window.width
						let distance = (1440 - this.window.width) * percent
						this.$refs.puppets.scrollLeft = distance
					}
				},
				intoIndex(){
					let tl = gsap.timeline({paused: true})
					bodyScrollLock.disableBodyScroll(document.body)
					tl.to('.loading .title-1 img', {rotate: 0, duration: 1.2, ease: "power3.out"}, 0)
					  .to('.loading .title-2 img', {y: 0, duration: 1, ease: "power3.out"}, 0)
					  .to('.loading .title-3 img', {y: 0, duration: 1, stagger: .2, ease: "power3.out"}, 0)
					  .to('.loading .content', {opacity: 1, duration: 1, ease: "power3.out"}, 1)
					  .to('.loading .tips', {opacity: 1, duration: 1, ease: "power3.out"}, 1)
					  .add(()=>{
						bodyScrollLock.enableBodyScroll(document.body)
					  })
					imagesLoaded( document.querySelector('.loading'), ( instance ) => {
						tl.play()
					});
				}
			},
			watch: {
				question(val){
					...
					this.$refs.puppets.scrollLeft = 0
				},
				playerLoad(val){
					if(val === 'loading'){
						this.isLoading = true
