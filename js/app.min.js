...
var global={init:function(){}};var index={init:function(){new Vue({el:"#wrapper",data:{puppets:[],player:{time:0,lyrics:null,egg:null,id:0,state:"load",current:null},question:"0",timeline:{lyrics:null},window:{width:0,height:0},clientX:0,isLoading:!0},created(){this.fetchData(),window.addEventListener("resize",this.handleResize),this.handleResize()},computed:{puppetsGroup(){switch(this.question){case"1":return _.chain(this.puppets).groupBy("gsx$independentpos.$t").value();case"2":return _.chain(this.puppets).groupBy("gsx$warspos.$t").value();case"3":return _.chain(this.puppets).groupBy("gsx$flagpos.$t").value();case"0":return _.chain(this.puppets).groupBy("gsx$originpos.$t").value()}}},methods:{async fetchData(){try{const t=await fetch("https://spreadsheets.google.com/feeds/list/1bvYB1LQYhDZaxJ68adB956Y-uowWWokgVr_9yyj_odM/1/public/values?alt=json",{method:"GET"}),e=await t.json();this.puppets=e.feed.entry}catch(t){console.error(t)}finally{this.isLoading=!1}},kickPuppet(t){"0"!==this.question&&(this.player.id=String(t),this.infoDisplay(!0),this.playerData(),this.playerDisplay(!0),this.playerAudio(),this.playerLyrics(),this.player.egg&&this.eggDisplay(!0))},infoDisplay(t){if(t){let t=gsap.timeline();const e="#puppet-"+this.player.id;t.set(".puppet",{className:"puppet unactive"}).set(e,{className:"puppet active"})}else gsap.set(".puppet",{className:"puppet"})},playerData(){const t=this.puppets.find(t=>t.gsx$uuid.$t===this.player.id);switch(this.question){case"1":this.player.lyrics=t.gsx$independentlyrics.$t,this.player.time=t.gsx$independenttime.$t,this.player.egg=t.gsx$independentegg.$t;break;case"2":this.player.lyrics=t.gsx$warslyrics.$t,this.player.time=t.gsx$warstime.$t,this.player.egg=t.gsx$warsegg.$t;break;case"3":this.player.lyrics=t.gsx$flaglyrics.$t,this.player.time=t.gsx$flagtime.$t,this.player.egg=t.gsx$flagegg.$t}},playerDisplay(t){t?gsap.set(".player",{display:"flex"}):gsap.set(".player",{display:"none"})},eggDisplay(t){t?(gsap.set(".egg",{display:"block"}),this.$refs.eggContent.scrollTop=0):gsap.set(".egg",{display:"none"})},playerAudio(){gsap.timeline().add(()=>{this.player.current&&(this.player.current.pause(),this.player.current.currentTime=0),this.player.state="load",this.player.current=new Audio("audio/"+this.player.id+"-"+this.question+".mp3"),this.player.current.load()}).add(()=>{this.player.current.onloadeddata=(t=>{this.audioPlay()})})},audioPlay(){this.player.state="play",this.player.current.play(),this.timeline.lyrics.play()},audioPause(){this.player.state="pause",this.player.current.pause(),this.timeline.lyrics.pause()},audioStop(){this.audioPause(),this.player.current.currentTime=0,this.infoDisplay(!1),this.playerDisplay(!1),this.eggDisplay(!1),this.timeline.lyrics.kill()},playerLyrics(){this.timeline.lyrics&&this.timeline.lyrics.kill();let t=Number(this.player.time)+1;this.timeline.lyrics=gsap.fromTo(".player .lyrics .text",{x:"0%"},{x:"-100%",duration:t,ease:"none",paused:!0,onComplete:()=>{this.infoDisplay(!1),this.playerDisplay(!1),this.eggDisplay(!1)}})},stIndex(){ScrollTrigger.create({snap:0}),gsap.timeline({scrollTrigger:{trigger:".stage",start:"top top",end:"700% top",pin:!0,scrub:!0}}).from(".z-6",{opacity:0,duration:.1}).from(".intro-1 p",{stagger:1,opacity:0,y:30,duration:2}).to(".intro-1",{opacity:0,duration:.5}).from(".intro-2 h3",{opacity:0,y:30,duration:1}).from(".intro-2 figure",{stagger:.33,opacity:0,y:30,duration:1}).set(".z-5",{display:"block"}).set(".z-6",{display:"none"}).add(()=>{this.question="0"}).add(()=>{this.question="1"}).set(".z-4",{display:"block"},"+=3").set(".z-5",{display:"none"}).add(()=>{this.question="1"}).add(()=>{this.question="2"}).set(".z-3",{display:"block"},"+=3").set(".z-4",{display:"none"}).add(()=>{this.question="2"}).add(()=>{this.question="3"}).set(".z-2",{display:"block"},"+=3").set(".z-3",{display:"none"}).add(()=>{this.player.current&&this.audioStop()}),gsap.timeline({scrollTrigger:{trigger:".credit",start:"top bottom",end:"bottom bottom",scrub:!0}}).set(".puppets",{className:"puppets end"},0).from(".credit .info",{y:30,rotate:10,opacity:0,duration:.2},.8),gsap.from(".puppets .container",{duration:1,x:"-100%",scrollTrigger:{trigger:".loading",start:"top top",end:"bottom top",scrub:!0}})},handleResize(){this.window.width=window.innerWidth,this.window.height=window.innerHeight},hozMove(t){if(this.window.width<1440&&this.window.width>992){this.clientX=t.clientX;let e=this.clientX/this.window.width,i=(1440-this.window.width)*e;this.$refs.puppets.scrollLeft=i}}},watch:{question(t){this.player.current&&this.audioStop(),gsap.from(".puppets .container",{y:-50,opacity:0,duration:.25}),this.$refs.puppets.scrollLeft=0}},filters:{filterName(t){switch(t){case"1":return"統一";case"2":return"維持現狀";case"3":return"獨立";case"4":return"願意";case"5":return"不願意";case"6":return"拿";case"7":return"不拿";default:return""}}},mounted(){this.$nextTick(()=>{gsap.timeline().add(()=>{this.infoDisplay(!1),this.playerDisplay(!1),this.eggDisplay(!1)}).add(()=>{this.stIndex()})})}})}};
