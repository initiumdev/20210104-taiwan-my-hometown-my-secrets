	...
	var init = function(){
		new Vue({
					...
					switch(this.question){
						case '1':
							return _.chain(this.puppets)
									.groupBy('gsx$independentpos.$t')
									.value()
							break;
						case '2':
							return _.chain(this.puppets)
									.groupBy('gsx$warspos.$t')
									.value()
							break;
						case '3':
							return _.chain(this.puppets)
									.groupBy('gsx$flagpos.$t')
									.value()
							break;
						case '0':
							return _.chain(this.puppets)
									.groupBy('gsx$originpos.$t')
									.value()
							break;		
					}
				}
			...
			methods: {
				...
				playerData(){
					const data = this.puppets.find(i => i.gsx$uuid.$t === this.player.id)
					switch(this.question){
						case '1':
							this.player.lyrics = data.gsx$independentlyrics.$t
							this.player.time = data.gsx$independenttime.$t
							this.player.egg = data.gsx$independentegg.$t
							break;
						case '2':
							this.player.lyrics = data.gsx$warslyrics.$t
							this.player.time = data.gsx$warstime.$t
							this.player.egg = data.gsx$warsegg.$t
							break;
						case '3':
							this.player.lyrics = data.gsx$flaglyrics.$t
							this.player.time = data.gsx$flagtime.$t
							this.player.egg = data.gsx$flagegg.$t
							break;
					}
				},
				...
				playerAudio(){
					const audio = new Audio('audio/'+this.player.id+'-'+this.question+'.mp3');
				...
				playerLyrics(){
					...
					let time = Number(this.player.time) + 1
					this.timeline.lyrics = gsap.fromTo('.player .lyrics .text', {x: '0%'}, {x: '-100%', duration: time, ease: "none", onComplete: () => {
						this.infoDisplay(false)
						this.playerDisplay(false)
						this.eggDisplay(false)
					}})
				...
				stIndex(){
					...
					tl1.from('.z-6', {opacity: 0, duration: .1})
					...
					tl2.set('.puppets', {className: "puppets end"}, 0)
					  .from('.credit .info', {y: 30, rotate: 10, opacity: 0, duration: .2}, .8)

					gsap.from('.puppets .container', {
						duration: 1,
						x: '-100%',
						scrollTrigger: {
							trigger: ".loading",
				...
				hozMove(e){
					if(this.window.width < 1440 && this.window.width > 992){
			...
			watch: {
				question(val){
					...
					gsap.from('.puppets .container', {y: -50, opacity: 0, duration: .5})
